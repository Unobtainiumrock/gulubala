"""Shared application service for CLI, API, voice, and document flows."""
...
class CallCenterService:
    ...
    def start_demo_session(
        self, scenario_id: str, channel: str = "voice"
    ) -> dict[str, Any]:
    ...
    def route_intent(self, session_id: str, utterance: str) -> dict[str, Any]:
        ...
        session.metadata["needs_disambiguation"] = result.get(
    ...
    def submit_field(
        self, session_id: str, field_name: str, value: str, source: str = "caller"
    ) -> dict[str, Any]:
        session = self.get_session(session_id)
        workflow = self._require_workflow(session)
        result = self.engine.submit_field(
            session, workflow, field_name, value, source=source
        )
        self.store.save_session(session)
    ...
    def submit_supporting_document(
        self, session_id: str, document_text: str
    ) -> dict[str, Any]:
        session = self.get_session(session_id)
        workflow = self._require_workflow(session)
    ...
    def handle_user_turn(
        ...
        *,
        ...
        submissions = self.engine.attempt_multi_field_capture(
            session, workflow, utterance
        )
        ...
        if session.escalate:
            ...
            session = self.get_session(session.session_id)
            message = (
                f"I am handing this over to a human specialist. {summary['summary']}"
            )
        else:
    ...
    def _compare_document_against_state(
        self, session, extracted_fields: dict[str, str]
    ) -> list[str]:
        mismatches = []
