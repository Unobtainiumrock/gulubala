...
from tests.conftest import (
    make_service,
    stub_field_extractor,
    stub_multi_field_extractor,
)
...
class TestV4BatchExtraction:
    def test_stub_extracts_multiple_values(self):
        ...
        workflow = get_workflow("billing_dispute")
        fields = [
            workflow.get_field(n)
            for n in ["account_number", "charge_date", "charge_amount"]
        ]
    ...
    def test_free_text_uses_remaining_tokens(self):
        ...
        result = engine.multi_field_extractor(
            fields, "12345678 wrong charge on my bill"
        )
        assert result["account_number"] == "12345678"
...
class TestV5MultiFieldCapture:
    ...
    def test_partial_extraction_keeps_valid(self):
        ...
        results = engine.attempt_multi_field_capture(session, workflow, "03/10/2026")
        assert "charge_date" in session.validated_fields
    ...
    def test_escalation_on_max_retries(self):
        ...
        results = engine.attempt_multi_field_capture(session, workflow, "12")
    ...
    def test_no_fields_extracted_returns_empty(self):
        ...
        results = engine.attempt_multi_field_capture(session, workflow, "hello there")
        assert results == []
...
class TestV7Integration:
    def test_billing_dispute_multi_field_single_turn(self, monkeypatch):
        ...
        service.handle_user_turn(
            session.session_id, "I need to dispute a charge on my bill."
        )
